...
static __global__ void dequantize_block(const void * __restrict__ vx, dst_t * __restrict__ y,
    ...
    y[iy0 + 0]        = ggml_ac4k_cast<dst_t>(v.x);
    y[iy0 + y_offset] = ggml_ac4k_cast<dst_t>(v.y);
}

template <bool need_check>
static __global__ void dequantize_block_q8_0_f16(const void * __restrict__ vx, half * __restrict__ y, const int64_t k) {
#if __CUDA_ARCH__ >= GGML_AC4K_CC_PASCAL
...
#pragma unroll
    ...
    NO_DEVICE_CODE;
#endif // __CUDA_ARCH__ >= GGML_AC4K_CC_PASCAL
}
...
static __global__ void dequantize_block_mxfp4(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    dst_t * y = yy + i*QK_K + 32*ib + 4*il;
    const uint8_t  * q4 = x[ib].qs + 4*il;
    const float d = ggml_ac4k_e8m0_to_fp32(x[ib].e);
    for (int j = 0; j < 4; ++j) {
...
static __global__ void convert_unary(
    ...
    y[iy] = ggml_ac4k_cast<dst_t>(x[ix]);
}
...
to_fp16_cuda_t ggml_get_to_fp16_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q8_0:
            if (fp16_available(ggml_ac4k_info().devices[ggml_ac4k_get_device()].cc)) {
