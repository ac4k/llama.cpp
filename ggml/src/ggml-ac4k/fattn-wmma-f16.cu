...
__launch_bounds__(nwarps*ggml_ac4k_get_physical_warp_size(), 1)
...
#if defined(FLASH_ATTN_AVAILABLE) && (__CUDA_ARCH__ == GGML_AC4K_CC_VOLTA || (defined(GGML_HIP_ROCWMMA_FATTN) && defined(GGML_USE_WMMA_FATTN)))
...
#pragma unroll
                for (int k0 = 0; k0 < FATTN_KQ_STRIDE/2; k0 += warp_size) {
                    ...
                    KQ_max_new = ggml_ac4k_hmax2(KQ_max_new, KQ2_tmp[k0/warp_size]);
                }
                KQ_max_new = __half2half2(warp_reduce_max<warp_size>(ggml_ac4k_hmax(__low2half(KQ_max_new), __high2half(KQ_max_new))));
...
#pragma unroll
    ...
    GGML_UNUSED_VARS(Q, K, V, mask, sinks, KV_max, dst, dst_meta, scale,
        ...
        ne10, ne11, ne12, ne13,
              ...
              nb31, nb32, nb33);
    NO_DEVICE_CODE;
#endif // defined(FLASH_ATTN_AVAILABLE) && (__CUDA_ARCH__ == GGML_AC4K_CC_VOLTA || (defined(GGML_HIP_ROCWMMA_FATTN) && defined(GGML_USE_WMMA_FATTN)))
}
...
void ggml_ac4k_flash_attn_ext_wmma_f16_case(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * KQV = dst;

    constexpr int nwarps = 4;

    constexpr int frag_m = cols_per_block == 8 && D % 32 == 0 ? 32 : 16;
    const int warp_size = ggml_ac4k_info().devices[ggml_ac4k_get_device()].warp_size;
...
void ggml_ac4k_flash_attn_ext_wmma_f16(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ...
    if (prec != GGML_PREC_DEFAULT) {
        if (Q->ne[1] <= 32 || Q->ne[0] > 128) {
            ...
            switch (Q->ne[0]) {
                case 64:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case< 64, cols_per_block, float>(ctx, dst);
                    break;
                case 80:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case< 80, cols_per_block, float>(ctx, dst);
                    break;
                case 96:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case< 96, cols_per_block, float>(ctx, dst);
                    break;
                case 112:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case<112, cols_per_block, float>(ctx, dst);
                    break;
                case 128:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case<128, cols_per_block, float>(ctx, dst);
                    break;
                case 256:
                    ggml_ac4k_flash_attn_ext_wmma_f16_case<256, cols_per_block, float>(ctx, dst);
                    break;
                default:
    ...
    if (Q->ne[1] <= 8 && Q->ne[0] % warp_size == 0) {
        ...
        switch (Q->ne[0]) {
            case 64:
                ggml_ac4k_flash_attn_ext_wmma_f16_case< 64, cols_per_block, half>(ctx, dst);
                break;
            case 96:
                ggml_ac4k_flash_attn_ext_wmma_f16_case< 96, cols_per_block, half>(ctx, dst);
                break;
            case 128:
                ggml_ac4k_flash_attn_ext_wmma_f16_case<128, cols_per_block, half>(ctx, dst);
                break;
            case 256:
                ggml_ac4k_flash_attn_ext_wmma_f16_case<256, cols_per_block, half>(ctx, dst);
                break;
            default:
                GGML_ABORT("fatal error");
                break;
    ...
    if (Q->ne[1] <= 32) {
        ...
        switch (Q->ne[0]) {
            case 64:
                ...
                break;
            case 80:
                ggml_ac4k_flash_attn_ext_wmma_f16_case< 80, cols_per_block, half>(ctx, dst);
                break;
            case 96:
                ggml_ac4k_flash_attn_ext_wmma_f16_case< 96, cols_per_block, half>(ctx, dst);
                break;
            case 112:
                ggml_ac4k_flash_attn_ext_wmma_f16_case<112, cols_per_block, half>(ctx, dst);
                break;
            case 128:
                ggml_ac4k_flash_attn_ext_wmma_f16_case<128, cols_per_block, half>(ctx, dst);
                break;
