...
struct fattn_mma_f16_config<256, 256> {
    ...
    static int get_nbatch_combine_host(const int cc, const int ncols) {
        if (ggml_ac4k_highest_compiled_arch(cc) == GGML_AC4K_CC_TURING) {
    ...
    static constexpr __device__ int get_nbatch_combine_device(int ncols) {
#if __CUDA_ARCH__ == GGML_AC4K_CC_TURING
        return ncols <= 16 ? 128 : 64;
#else
        GGML_UNUSED(ncols);
        return 128;
#endif // __CUDA_ARCH__ == GGML_AC4K_CC_TURING
    }
};
...
static __device__ __forceinline__ void flash_attn_ext_f16_load_tile(
    ...
    if (use_cp_async) {
        ...
        const unsigned int tile_KV_32 = ggml_ac4k_cvta_generic_to_shared(tile_KV);
...
static __device__ __forceinline__ void flash_attn_ext_f16_load_mask(
    ...
    if (use_cp_async) {
        ...
        const unsigned int tile_mask_32 = ggml_ac4k_cvta_generic_to_shared(tile_mask);
...
void ggml_ac4k_flash_attn_ext_mma_f16_case(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
...
#define DECL_FATTN_MMA_F16_CASE(DKQ, DV, ncols1, ncols2)                          \
    template void ggml_ac4k_flash_attn_ext_mma_f16_case                           \
