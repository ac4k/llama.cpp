...
template<class op>
static void ggml_ac4k_op_bin_bcast(
...
void ggml_ac4k_op_repeat(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_bin_bcast<bin_bcast_cuda<op_repeat, 0>>(dst, dst->src[0], dst, nullptr, dst->src[0]->data, dst->data, ctx.stream());
}

void ggml_ac4k_op_add(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_bin_bcast<bin_bcast_cuda<op_add>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void ggml_ac4k_op_sub(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_bin_bcast<bin_bcast_cuda<op_sub>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void ggml_ac4k_op_mul(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_bin_bcast<bin_bcast_cuda<op_mul>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void ggml_ac4k_op_div(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_bin_bcast<bin_bcast_cuda<op_div>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

template <float (*op)(const float, const float), int n_fuse>
static void ggml_ac4k_op_fused_binbcast_impl(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
...
void ggml_ac4k_op_fused_add(ggml_backend_ac4k_context & ctx, ggml_tensor * dst, int n_fuse) {
    ...
    switch (n_fuse) {
        case 2:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 2>(ctx, dst);
            break;
        case 3:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 3>(ctx, dst);
            break;
        case 4:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 4>(ctx, dst);
            break;
        case 5:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 5>(ctx, dst);
            break;
        case 6:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 6>(ctx, dst);
            break;
        case 7:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 7>(ctx, dst);
            break;
        case 8:
            ggml_ac4k_op_fused_binbcast_impl<op_add, 8>(ctx, dst);
            break;
        default:
