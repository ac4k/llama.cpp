...
static void launch_topk_moe_cuda(ggml_backend_ac4k_context & ctx,
                                 const float *               logits,
                                 float *                     weights,
                                 int32_t *                   ids,
                                 const int                   n_rows,
...
void ggml_ac4k_op_topk_moe(ggml_backend_ac4k_context & ctx,
...
bool ggml_ac4k_should_use_topk_moe(const ggml_tensor * softmax, const ggml_tensor * weights, const ggml_tensor * clamp) {
    float scale    = 1.0f;
    float max_bias = 0.0f;
...
std::initializer_list<enum ggml_op> ggml_ac4k_topk_moe_ops(bool norm, bool delayed_softmax) {
