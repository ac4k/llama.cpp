...
static int get_mmq_x_max_host(const int cc) {
    return (amd_mfma_available(cc) || turing_mma_available(cc)) ? 128 :
        GGML_AC4K_CC_IS_NVIDIA(cc) && ggml_ac4k_highest_compiled_arch(cc) >= GGML_AC4K_CC_VOLTA ?
...
#ifdef GGML_AC4K_FORCE_MMQ
    return 128;
#else // GGML_AC4K_FORCE_MMQ
    return MMQ_DP4A_MAX_BATCH_SIZE;
#endif // GGML_AC4K_FORCE_MMQ
#else // __CUDA_ARCH__ >= GGML_AC4K_CC_VOLTA
    return 64;
...
static int get_mmq_y_host(const int cc) {
    return GGML_AC4K_CC_IS_AMD(cc) ? (GGML_AC4K_CC_IS_RDNA1(cc) ? 64 : 128) :
        ((GGML_AC4K_CC_IS_NVIDIA(cc) && ggml_ac4k_highest_compiled_arch(cc) >= GGML_AC4K_CC_VOLTA) ? 128 : 64);
}
...
static constexpr __device__ int mmq_get_nwarps_device() {
    ...
    return 8;
#else
    return 256/ggml_ac4k_get_physical_warp_size();
#endif // AMD_MFMA_AVAILABLE
}
...
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
        x_df[i*MMQ_MMA_TILE_X_K_Q8_1                 + kbxd] = ggml_ac4k_e8m0_to_fp32(bxi->e)*0.5f;
#else
        x_df[i*(MMQ_TILE_NE_K/QI_MXFP4) + i/QI_MXFP4 + kbxd] = ggml_ac4k_e8m0_to_fp32(bxi->e)*0.5f;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }
}
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q2_K(
    ...
    constexpr int nrows = ggml_ac4k_get_physical_warp_size() / threads_per_row;
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
    ...
    constexpr int              warp_size  = ggml_ac4k_get_physical_warp_size();
    constexpr int              nwarps     = mmq_get_nwarps_device();
    constexpr int              qk         = ggml_ac4k_type_traits<type>::qk;
...
template <ggml_type type, int mmq_x, bool need_check>
    ...
    __launch_bounds__(ggml_ac4k_get_physical_warp_size()*mmq_get_nwarps_device(), 2)
#endif // defined(RDNA4) || defined(RDNA3) || defined(RDNA2) || defined(CDNA) || defined(GCN)
#else
#if __CUDA_ARCH__ >= GGML_AC4K_CC_VOLTA
    __launch_bounds__(ggml_ac4k_get_physical_warp_size()*mmq_get_nwarps_device(), 1)
#else
    __launch_bounds__(ggml_ac4k_get_physical_warp_size()*mmq_get_nwarps_device(), 2)
#endif // __CUDA_ARCH__ >= GGML_AC4K_CC_VOLTA
#endif // defined(GGML_USE_HIP)
...
#if (defined(GGML_USE_HIP) && !defined(CDNA)) || __CUDA_ARCH__ < GGML_AC4K_CC_VOLTA
    {
...
#endif // (defined(GGML_USE_HIP) && !defined(CDNA3)) || __CUDA_ARCH__ < GGML_AC4K_CC_VOLTA
...
static void launch_mul_mat_q(ggml_backend_ac4k_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    const int nsm = ggml_ac4k_info().devices[id].nsm;
    ...
    ggml_ac4k_pool & pool = ctx.pool(id);
    ggml_ac4k_pool_alloc<float> tmp_fixup(pool);
    if (fixup_needed) {
...
void mul_mat_q_case(ggml_backend_ac4k_context & ctx, const mmq_args & args, cudaStream_t stream) {
    const int    id     = ggml_ac4k_get_device();
    const int    cc     = ggml_ac4k_info().devices[id].cc;
    const size_t smpbo  = ggml_ac4k_info().devices[id].smpbo;
    const int warp_size = ggml_ac4k_info().devices[id].warp_size;
    const int nwarps    = mmq_get_nwarps_host(cc, warp_size);
