...
#ifdef GGML_AC4K_USE_CUB
#    include <cub/cub.cuh>
using namespace cub;
#endif  // GGML_AC4K_USE_CUB
...
#ifdef GGML_AC4K_USE_CUB
static void argsort_f32_i32_cuda_cub(ggml_ac4k_pool & pool,
                                     const float *    x,
                                     int *            dst,
                                     const int        ncols,
                                     const int        nrows,
                                     ...
                                     cudaStream_t     stream) {
    ggml_ac4k_pool_alloc<int>   temp_indices_alloc(pool, ncols * nrows);
    ggml_ac4k_pool_alloc<float> temp_keys_alloc(pool, ncols * nrows);
    ggml_ac4k_pool_alloc<int>   offsets_alloc(pool, nrows + 1);
    ...
    ggml_ac4k_pool_alloc<uint8_t> temp_storage_alloc(pool, temp_storage_bytes);
...
template<typename T>
static inline __device__ void ggml_ac4k_swap(T & a, T & b) {
    T tmp = a;
    a = b;
    b = tmp;
}
            ...
            if (ixj > col) {
                if ((col & k) == 0) {
                    if (dst_row[col] >= ncols ||
                        ...
                        ggml_ac4k_swap(dst_row[col], dst_row[ixj]);
                    }
                } else {
...
static void argsort_f32_i32_cuda_bitonic(const float *   x,
    ...
    GGML_ASSERT(shared_mem <= ggml_ac4k_info().devices[ggml_ac4k_get_device()].smpb);
...
void ggml_ac4k_op_argsort(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ...
    const int    ncols_pad      = next_power_of_2(ncols);
    const size_t shared_mem     = ncols_pad * sizeof(int);
    const size_t max_shared_mem = ggml_ac4k_info().devices[ggml_ac4k_get_device()].smpb;
