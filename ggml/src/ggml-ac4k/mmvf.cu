...
static __global__ void mul_mat_vec_f(
        const T * __restrict__ x, const float * __restrict__ y, const int32_t * __restrict__ ids, const ggml_ac4k_mm_fusion_args_device fusion, float * __restrict__ dst,
    ...
    constexpr int warp_size   = ggml_ac4k_get_physical_warp_size();
...
#pragma unroll
            for (int j = 0; j < ncols_dst; ++j) {
                ...
                ggml_ac4k_mad(sumf[j], tmpx.x, tmpy.x);
                ggml_ac4k_mad(sumf[j], tmpx.y, tmpy.y);

                if constexpr (has_fusion) {
                    if (use_gate) {
                        ggml_ac4k_mad(sumf_gate[j], tmpx_gate.x, tmpy.x);
                        ggml_ac4k_mad(sumf_gate[j], tmpx_gate.y, tmpy.y);
                    }
                }
            }
        }
...
//TODO: add support for ggml_ac4k_mad for hip_bfloat162
...
#pragma unroll
            for (int j = 0; j < ncols_dst; ++j) {
                const float2 tmpy = y2[j*stride_col_y2 + col2];
                const float tmpx0 = ggml_ac4k_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx)[0]);
                const float tmpx1 = ggml_ac4k_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx)[1]);
                ggml_ac4k_mad(sumf[j], tmpx0, tmpy.x);
                ggml_ac4k_mad(sumf[j], tmpx1, tmpy.y);

                if constexpr (has_fusion) {
                    if (use_gate) {
                        const float tmpx0_gate = ggml_ac4k_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx_gate)[0]);
                        const float tmpx1_gate = ggml_ac4k_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx_gate)[1]);
                        ggml_ac4k_mad(sumf_gate[j], tmpx0_gate, tmpy.x);
                        ggml_ac4k_mad(sumf_gate[j], tmpx1_gate, tmpy.y);
                    }
                }
            }
        }
        ...
        if (use_gate) {
            ...
            switch (glu_op) {
                case GGML_GLU_OP_SWIGLU:
                    value *= ggml_ac4k_op_silu_single(gate_value);
                    break;
                case GGML_GLU_OP_GEGLU:
                    value *= ggml_ac4k_op_gelu_single(gate_value);
                    break;
                case GGML_GLU_OP_SWIGLU_OAI: {
                    value = ggml_ac4k_op_swiglu_oai_single(gate_value, value);
                    break;
                }
                default:
                    break;
...
static void mul_mat_vec_f_switch_fusion(
        const T * x, const float * y, const int32_t * ids, const ggml_ac4k_mm_fusion_args_device fusion, float * dst,
...
void launch_mul_mat_vec_f_cuda(
    ...
    const int device = ggml_ac4k_get_device();
    const int warp_size = ggml_ac4k_info().devices[device].warp_size;

    int64_t block_size_best = warp_size;
    int64_t niter_best      = (ncols + 2*warp_size - 1) / (2*warp_size);
    int64_t max_block_size  = 256;
    if(ggml_ac4k_info().devices[device].cc > GGML_AC4K_CC_OFFSET_AMD && ggml_ac4k_info().devices[device].cc < GGML_AC4K_CC_RDNA1) {
        max_block_size = 128;
    }
...
void ggml_ac4k_mul_mat_vec_f(ggml_backend_ac4k_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst,
    const ggml_ac4k_mm_fusion_args_host * fusion) {
    ...
    ggml_ac4k_mm_fusion_args_device fusion_local{};
...
void ggml_ac4k_op_mul_mat_vec_f(
    ...
    // ggml_ac4k_op provides single, contiguous matrices
    ...
    ggml_ac4k_mm_fusion_args_device empty{};
    switch (src0->type) {
        case GGML_TYPE_F32: {
...
bool ggml_ac4k_should_use_mmvf(enum ggml_type type, int cc, const int64_t * src0_ne, int64_t ne11) {
    if (src0_ne[0] % 2 != 0) {
        return false;
    }
    switch (type) {
        case GGML_TYPE_F32:
            if (GGML_AC4K_CC_IS_NVIDIA(cc)) {
                if (ampere_mma_available(cc)) {
                    return ne11 <= 3;
                }
                if (cc >= GGML_AC4K_CC_TURING) {
                    return ne11 <= 4;
                }
                return ne11 <= 3;
            } else if (GGML_AC4K_CC_IS_AMD(cc)) {
        ...
        case GGML_TYPE_F16:
            if (GGML_AC4K_CC_IS_NVIDIA(cc)) {
                ...
                if (fp16_mma_hardware_available(cc)) {
                    if (GGML_AC4K_CC_IS_RDNA3(cc) || GGML_AC4K_CC_IS_RDNA4(cc)) {
                        return ne11 <= 5;
                    }
                    return ne11 <= 2;
                }
