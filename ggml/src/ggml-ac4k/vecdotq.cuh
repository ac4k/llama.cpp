...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        sumi = ggml_ac4k_dp4a(vi0, u[2*i+0], sumi);
        sumi = ggml_ac4k_dp4a(vi1, u[2*i+1], sumi);
    }
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        sumi = ggml_ac4k_dp4a(vi0, u[2*i+0], sumi); // SIMD dot product of quantized values
        ...
        sumi = ggml_ac4k_dp4a(vi1, u[2*i+1], sumi); // SIMD dot product of quantized values
    }
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        // SIMD dot product of quantized values
        sumi = ggml_ac4k_dp4a(v[i], u[i], sumi);
    }
...
#pragma unroll
    for (int l = 0; l < VDR_MXFP4_Q8_1_MMVQ; ++l) {
        ...
        sumi = ggml_ac4k_dp4a(v.x, q8[l + 0], sumi);
        sumi = ggml_ac4k_dp4a(v.y, q8[l + 4], sumi);
    }

    const float d = ggml_ac4k_e8m0_to_fp32(bq4->e) * 0.5f * __low2float(bq8_1->ds);
    return d * sumi;
}
...
#pragma unroll
    for (int i = 0; i < QR2_K; ++i) {
        ...
        sumf_d += d8[i] * (ggml_ac4k_dp4a(vi, u[i], 0) * (sc & 0xF)); // SIMD dot product

        // fill int with 4x m
        int m = sc >> 4;
        m |= m <<  8;
        m |= m << 16;
        sumf_m += d8[i] * ggml_ac4k_dp4a(m, u[i], 0); // multiply constant q2_K part with sum of q8_1 values
    }
...
#pragma unroll
        for (int i = i0; i < i0 + QI8_1/2; ++i) {
            sumi_d0 = ggml_ac4k_dp4a(v[i], u[i], sumi_d0);
        }
        sumf_d8 += dm2f0.x * sumi_d0;

#pragma unroll
        for (int i = i0 + QI8_1/2; i < i0 + QI8_1; ++i) {
            sumi_d1 = ggml_ac4k_dp4a(v[i], u[i], sumi_d1);
        }
...
#pragma unroll
            for (int i = i0; i < i0 + QI8_1/2; ++i) {
                sumi_m0 = ggml_ac4k_dp4a(0x01010101, u[i], sumi_m0);
            }
...
#pragma unroll
            for (int i = i0 + QI8_1/2; i < i0 + QI8_1; ++i) {
                sumi_m1 = ggml_ac4k_dp4a(0x01010101, u[i], sumi_m1);
            }
...
#pragma unroll
    for (int i = 0; i < QR3_K; ++i) {
        ...
        sumf += d8[i] * (ggml_ac4k_dp4a(vi, u[i], 0) * sc); // SIMD dot product
    }
...
#pragma unroll
        for (int i = i0; i < i0 + QI8_1/2; ++i) {
            sumi_sc = ggml_ac4k_dp4a(v[i], u[i], sumi_sc); // SIMD dot product
        }
...
#pragma unroll
    for (int i = 0; i < QR4_K; ++i) {
        ...
        const int dot1 = ggml_ac4k_dp4a(v1i, u[2*i+1], ggml_ac4k_dp4a(v0i, u[2*i+0], 0)); // SIMD dot product
        const int dot2 = ggml_ac4k_dp4a(0x01010101, u[2*i+1], ggml_ac4k_dp4a(0x01010101, u[2*i+0], 0)); // sum of u
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = ggml_ac4k_dp4a((v[j] >> (4*i)) & 0x0F0F0F0F, u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
    for (int i = 0; i < QR5_K; ++i) {
        ...
        const int dot1 = ggml_ac4k_dp4a(v0i, u[2*i+0], ggml_ac4k_dp4a(v1i, u[2*i+1], 0)); // SIMD dot product
        const int dot2 = ggml_ac4k_dp4a(0x01010101, u[2*i+0], ggml_ac4k_dp4a(0x01010101, u[2*i+1], 0)); // sum of u
...
#pragma unroll
        for (int j = 0; j < QI8_1; ++j) {
            sumi_d = ggml_ac4k_dp4a(v[i*QI8_1 + j], u[i*QI8_1 + j], sumi_d); // SIMD dot product
        }
...
#pragma unroll
        for (int i = i0; i < i0 + 2; ++i) {
            sumi_d.x = ggml_ac4k_dp4a(v[2*i+0], u[2*i+0], sumi_d.x); // SIMD dot product
            sumi_d.x = ggml_ac4k_dp4a(v[2*i+1], u[2*i+1], sumi_d.x); // SIMD dot product

            sumi_d.y = ggml_ac4k_dp4a(v[2*i+4], u[2*i+4], sumi_d.y); // SIMD dot product
            sumi_d.y = ggml_ac4k_dp4a(v[2*i+5], u[2*i+5], sumi_d.y); // SIMD dot product
        }
...
#pragma unroll
    for (int k0 = 0; k0 < 8; k0 += 2) {
        ...
        sumi = ggml_ac4k_dp4a(grid0, u0, sumi);

        const int signs1 = __vcmpne4(((signs_packed & 0x30) << 3) | ((signs_packed & 0xC0) << 17), 0x00000000);
        const int grid1 = __vsub4(grid_pos[1] ^ signs1, signs1);
        const int u1 = get_int_b4(bq8_1[iqs/2].qs, k0 + 1);
        sumi = ggml_ac4k_dp4a(grid1, u1, sumi);
    }
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        ...
        if (l0 < 4) {
            sumi0 = ggml_ac4k_dp4a(grid_l, u0, sumi0);
            sumi0 = ggml_ac4k_dp4a(grid_h, u1, sumi0);
        } else {
            sumi1 = ggml_ac4k_dp4a(grid_l, u0, sumi1);
            sumi1 = ggml_ac4k_dp4a(grid_h, u1, sumi1);
        }
    }
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        ...
        sumi = ggml_ac4k_dp4a(grid_l, u0, sumi);
        sumi = ggml_ac4k_dp4a(grid_h, u1, sumi);
    }
...
#pragma unroll
    for (int l0 = 0; l0 < 8; l0 += 2) {
        ...
        sumi[l0/4] = ggml_ac4k_dp4a(grid0, u0, sumi[l0/4]);
        sumi[l0/4] = ggml_ac4k_dp4a(grid1, u1, sumi[l0/4]);

        const float delta = -1.0f + IQ1M_DELTA - (qhl & 0x08) * (2.0f*IQ1M_DELTA/0x08);
        int sumy = 0;
        sumy = ggml_ac4k_dp4a(u0, 0x01010101, sumy);
        sumy = ggml_ac4k_dp4a(u1, 0x01010101, sumy);
...
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        ...
        sumi = ggml_ac4k_dp4a(v.x, u0, sumi);
        sumi = ggml_ac4k_dp4a(v.y, u1, sumi);
    }
