...
static __device__ __forceinline__ float op_gelu(float x) {
    return ggml_ac4k_op_gelu_single(x);
}
...
static __device__ __forceinline__ float op_silu(float x) {
    return ggml_ac4k_op_silu_single(x);
}
...
void ggml_ac4k_op_unary(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
...
void ggml_ac4k_op_abs(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_abs>(ctx, dst);
}

void ggml_ac4k_op_sgn(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_sgn>(ctx, dst);
}

void ggml_ac4k_op_neg(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_neg>(ctx, dst);
}

void ggml_ac4k_op_step(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_step>(ctx, dst);
}

void ggml_ac4k_op_gelu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_gelu>(ctx, dst);
}

void ggml_ac4k_op_gelu_erf(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_gelu_erf>(ctx, dst);
}

void ggml_ac4k_op_gelu_quick(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_gelu_quick>(ctx, dst);
}

void ggml_ac4k_op_silu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_silu>(ctx, dst);
}

void ggml_ac4k_op_tanh(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_tanh>(ctx, dst);
}

void ggml_ac4k_op_relu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_relu>(ctx, dst);
}

void ggml_ac4k_op_sigmoid(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_sigmoid>(ctx, dst);
}

void ggml_ac4k_op_hardsigmoid(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_hardsigmoid>(ctx, dst);
}

void ggml_ac4k_op_hardswish(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_hardswish>(ctx, dst);
}

void ggml_ac4k_op_exp(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_exp>(ctx, dst);
}

void ggml_ac4k_op_sqr(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_sqr>(ctx, dst);
}

void ggml_ac4k_op_sqrt(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_sqrt>(ctx, dst);
}

void ggml_ac4k_op_sin(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_sin>(ctx, dst);
}

void ggml_ac4k_op_cos(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_cos>(ctx, dst);
}

void ggml_ac4k_op_log(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_log>(ctx, dst);
}

void ggml_ac4k_op_elu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary<op_elu>(ctx, dst);
}
/* gated ops */
...
template <float (*op)(float)>
void ggml_ac4k_op_unary_gated(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
...
void ggml_ac4k_op_reglu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary_gated<op_relu>(ctx, dst);
}

void ggml_ac4k_op_geglu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary_gated<op_gelu>(ctx, dst);
}

void ggml_ac4k_op_swiglu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary_gated<op_silu>(ctx, dst);
}

void ggml_ac4k_op_geglu_erf(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary_gated<op_gelu_erf>(ctx, dst);
}

void ggml_ac4k_op_geglu_quick(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    ggml_ac4k_op_unary_gated<op_gelu_quick>(ctx, dst);
}
...
static __global__ void swiglu_oai_kernel(const T * x, const T * g, T * dst, const int64_t k, const int64_t n, const int64_t o0, const int64_t o1, float alpha, float limit) {
    ...
    dst[i] = ggml_ac4k_op_swiglu_oai_single(xi, gi, alpha, limit);
}
...
void ggml_ac4k_op_swiglu_oai(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    void * src0_d = src0->data;
    void * src1_d = src1 ? src1->data : src0->data;
...
static __global__ void xielu_kernel(const T * x, T * dst, const int k, float alpha_n, float alpha_p, float beta, float eps) {
    ...
    const float xi = ggml_ac4k_cast<float>(x[i]);
...
void ggml_ac4k_op_xielu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const void * src0_d = src0->data;
    void * dst_d = dst->data;
    cudaStream_t stream = ctx.stream();
...
void ggml_ac4k_op_silu_back(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
...
void ggml_ac4k_op_leaky_relu(ggml_backend_ac4k_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const void * src0_d = src0->data;
    void * dst_d = dst->data;
    cudaStream_t stream = ctx.stream();
