...
static __device__ __forceinline__ int ggml_ac4k_movmatrix(const int x) {
    int ret = 0;
...
static __device__ __forceinline__ half2 ggml_ac4k_movmatrix(const half2 x) {
    half2 ret;
    *((int *) &ret) = ggml_ac4k_movmatrix(*((const int *) &x));
    return ret;
}

namespace ggml_ac4k_mma {
...
#pragma unroll
    ...
    static __device__ __forceinline__ tile<8, 8, half2> get_transposed(const tile<16, 4, half2> & t) {
        tile<8, 8, half2> ret;
        ret.x[0] = ggml_ac4k_movmatrix(t.x[0]);
        ret.x[1] = ggml_ac4k_movmatrix(t.x[1]);
...
#pragma unroll
    ...
    static __device__ __forceinline__ void mma(
...
#ifdef TURING_MMA_AVAILABLE
#if __CUDA_ARCH__ >= GGML_AC4K_CC_AMPERE
        ...
        asm("mma.sync.aligned.m8n8k16.row.col.s32.s8.s8.s32 {%0, %1}, {%2}, {%3}, {%0, %1};"
            : "+r"(D.x[2]), "+r"(D.x[3])
            : "r"(A.x[1]), "r"(B.x[0]));
#endif // __CUDA_ARCH__ >= GGML_AC4K_CC_AMPERE
#else
